...
8. Continue watching until the maintainer explicitly stops the review or the
   watched driver tmux pane/session exits -- those are the ONLY stop conditions.
   Plan completion is NOT a stop condition: a closed/completed plan thread, a
   driver waiting at a maintainer prompt, blocker prompt, or other idle input
   prompt, and a maintainer decision picker are all still an active watched
   session, so keep watching `master`. Tear down watcher processes only once the
   watched pane/session has exited or the maintainer ends the review.
