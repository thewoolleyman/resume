...
## Scenario: Interactive mode shows a loading indicator while data loads

Given governed resume data has not finished loading

When a visitor opens the interactive resume

Then a loading indicator is shown until the governed data loads and the resume renders
...
Then matching is case-insensitive, partial matches are shown, clearing the query restores the default ordered view, and a term with no matches shows a no-results state without leaving interactive mode
...
Then the item remains visible, the level indicator shows a diagnostic label such as `unknown level`, and the app exposes an implementation-visible diagnostic rather than silently dropping the item
...
Then the page title is `Chad Woolley - Resume`, a description meta tag is present, viewport metadata uses `width=device-width, initial-scale=1, shrink-to-fit=no` or a documented equivalent, favicon and app icons (or their documented replacements) are served, robots and canonical behavior is consistent with the preview-non-index rule, the PWA manifest decision is documented or implemented, and the layout has no horizontal scroll on supported viewports
