...
The inventory MUST contain a table with at least these columns: discipline, source inspiration, disposition, local enforcement artifact, aggregate-check coverage, and notes. The disposition value MUST be exactly one of adopted, locally adapted, deferred, or rejected. Adopted and locally adapted rows MUST cite a local TypeScript, Svelte, Bun, Vercel, GitHub, hook, work-item, or documentation mechanism that enforces the practice. Deferred and rejected rows MUST state the reason and the condition for revisiting the decision.
...
The inventory MUST preserve the standalone boundary: adopted discipline MAY be inspired by sibling livespec repositories, but runtime code, tests, CI, hooks, build scripts, and deployment MUST NOT require sibling checkouts, Python-only fleet tooling, or non-committed local state. `bun run check` MUST verify that `.ai/discipline-adoption.md` exists before the first non-trivial implementation merge, that it contains the baseline rows above, and that adopted or locally adapted rows cite existing local commands, workflows, hooks, work-item records, or documentation paths.
...
Then the inventory classifies every seed-listed discipline as adopted, locally adapted, deferred, or rejected and names the standalone TypeScript/Svelte/Bun/Vercel/GitHub mechanism that enforces any adopted or adapted practice
...
Then Svelte validation, TypeScript validation, linting, formatting, tests, coverage, property/fuzz gates, Result/ROP enforcement gates, discipline-inventory checks, and scenario coverage checks run through one documented entry point
