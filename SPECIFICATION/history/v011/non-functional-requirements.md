...
Before implementation planning claims the livespec fleet discipline has been adopted, the repository MUST maintain a local discipline-adoption inventory. The inventory MUST enumerate the applicable livespec fleet and livespec-dev-tooling practices considered for this project, classify each practice as adopted, locally adapted, deferred, or rejected, and cite the local TypeScript, Svelte, Bun, Vercel, GitHub, hook, or documentation mechanism that enforces each adopted or adapted practice.

The inventory MUST preserve the standalone boundary: adopted discipline MAY be inspired by sibling livespec repositories, but runtime code, tests, CI, hooks, build scripts, and deployment MUST NOT require sibling checkouts, Python-only fleet tooling, or non-committed local state. When the aggregate check can verify an inventory entry mechanically, `bun run check` MUST fail if the cited local command, hook, workflow, or documentation path is missing.
...
For UI behavior, the preferred red test is a Playwright test that observes user-visible behavior. A lower-level integration test MAY be the red test only when the behavior is not browser-observable, when it supports a separately mapped Playwright scenario, or when a documented exception explains why integration coverage is the correct top-level test. For pure data transforms, search, grounding, and contract logic, the preferred red test is a Vitest unit or property test.

### Top-of-pyramid discipline

Every load-bearing `## Scenario:` heading in `SPECIFICATION/scenarios.md` MUST be mapped to one or more repository test identifiers before the scenario is treated as implemented. Product and browser scenarios MUST default to Playwright end-to-end coverage. Integration tests MAY satisfy a load-bearing scenario only when the scenario is explicitly non-browser, when the E2E surface is redundant with a separately mapped Playwright scenario, or when the mapping documents why integration coverage is the correct top-level test.

The repository MUST maintain a scenario coverage mapping in committed configuration or data. The aggregate check MUST verify that every load-bearing scenario has a mapping, every mapped test identifier resolves to an existing test, later-phase scenarios are excluded until activated by a future proposed change, and stale mappings fail the check.
...
| Scenario coverage | Verify load-bearing scenario-to-test mappings. |
...
The aggregate check or a documented fast hook MUST fail when committed files match prohibited private-memory or hidden tool-state patterns, when an `.ai/*.md` file is not indexed from `AGENTS.md`, or when `AGENTS.md` references a missing `.ai` note. The rule MUST allow ordinary tool configuration files that are not private memory when those paths are explicitly documented as configuration rather than memory.
...
Pure parsing, normalization, markdown-to-text projection, search, filtering, sorting, citation, grounding, and MCP contract modules MUST have deterministic property-based or fuzz-style tests when the input space is broader than a few fixed examples. These tests MUST run through the aggregate check, MUST use Bun/Vitest-compatible tooling, and MUST run without network access. Any intentional exemption MUST be documented next to the module or in this file with a rationale.
...
When a contributor reviews the discipline-adoption inventory

Then the inventory classifies the practice as adopted, locally adapted, deferred, or rejected and names the standalone TypeScript/Svelte/Bun/Vercel/GitHub mechanism that enforces any adopted or adapted practice
...
Then the scenario coverage mapping links it to one or more existing test identifiers, product/browser scenarios default to Playwright coverage, and `bun run check` fails on missing, stale, or wrongly included mappings
...
Then Svelte validation, TypeScript validation, linting, formatting, tests, coverage, property/fuzz gates, Result/ROP enforcement gates, and scenario coverage checks run through one documented entry point
...
Given a committed file persists private local memory in a hidden tool-state location, an `.ai/*.md` note is missing from `AGENTS.md`, or `AGENTS.md` references a missing `.ai` note
