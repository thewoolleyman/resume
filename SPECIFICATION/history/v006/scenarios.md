...
Then the app rejects the source and renders the visitor-facing error state with the navigation and header shell still visible, rather than rendering partial or guessed resume data
...
Then the sticky navigation bar and centered header shell are visible immediately and a loading indicator is shown in the content area until the governed data loads and the resume renders
...
Then matching is case-insensitive, partial matches are shown in canonical item order, clearing the query restores the default ordered view, and a term with no matches shows an explicit no-results state without leaving interactive mode
...
Then the app keeps the navigation and header shell visible and shows an explicit error state written for visitors rather than a blank page or a leaked diagnostic
