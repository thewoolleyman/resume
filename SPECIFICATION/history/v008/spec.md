...
Phase 1 MUST have exactly one canonical governed data source. It MUST be a single, version-controlled document stored in this repository in a structured, human-readable, machine-parseable format (for example YAML or JSON). The repository MUST document the canonical source path so there is no ambiguity about which file is authoritative; a phase-1 implementation MUST NOT split resume facts across multiple competing source files.
