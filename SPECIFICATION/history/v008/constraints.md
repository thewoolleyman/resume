...
So this parity does not depend on a live external fetch, a verbatim snapshot of the predecessor's production content MUST be committed into this repository as the authoritative import material, with its provenance (source URL, retrieval date, upstream `Last-Modified`, and content hash) recorded per `spec.md` §"Governed data source and predecessor import (phase 1)". The committed snapshot, not the live URL, is what the build reads.
