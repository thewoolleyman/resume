...
Dates MUST render month/year as `M.YYYY` (a one- or two-digit month number with no zero-padding and a four-digit year). A present start date MUST render as its `M.YYYY` value followed immediately by the predecessor's start/end separator — a non-breaking space and a hyphen (`&nbsp;-`) — so the date column matches the predecessor exactly (for example `7.2001&nbsp;-`). A missing start with a present end MUST render `until` in the start position. A present start with a missing end MUST render `current` in the end position. A missing start AND end MUST render the item as `current` in the end position with nothing in the start position; this both-missing case overrides the missing-start-renders-`until` rule.
