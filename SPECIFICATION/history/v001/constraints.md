...
The project SHOULD use the current TypeScript ecosystem rather than copying Python-specific livespec tooling. Bun, Vitest, and Playwright are preferred tools unless a future proposed change selects different TypeScript-native equivalents.

## Framework and deployment

The app MUST be deployable to Vercel. Framework choices MUST preserve Vercel production, preview, and development workflows. If the implementation chooses Next.js, Vercel framework defaults SHOULD be used unless a requirement makes a custom setup necessary.

Production MUST serve from `resume.thewoolleyweb.com`. Preview URLs MUST be treated as non-production environments and MUST NOT be indexed or presented as canonical resume URLs.
...
The governed resume data is the authority for resume facts. UI copy, static rendering, AI answers, tests, and MCP responses MUST derive from that data or from this specification. Duplicated resume facts in components, prompts, tests, or documentation MUST be avoided unless the duplicate is an assertion fixture whose purpose is explicit.
...
## Local memory and agent state

Repository automation and agent workflows MUST NOT persist private local memories in hidden tool state. Any durable agent-facing project notes MUST live under `.ai/*.md` and be referenced from `AGENTS.md` when such files are introduced.
