...
When that production content is imported into the single canonical governed data source at `data/resume.yml`
...
Then each item receives a stable identifier composed deterministically per `spec.md` §"Stable item identifiers" — the base section slug, a single hyphen, and the item title slug (`<section-slug>-<title-slug>`), with item-level `-2`/`-3` collision suffixes appended to the composed identifier in governed data order and section-level collision suffixes excluded — so that, for example, the `Senior Software Engineer, Pivotal` item in `Job History` receives the public, deep-linkable anchor `job-history-senior-software-engineer-pivotal`, and the identifiers stay stable when items are reordered
...
## Scenario: Phase-1 data load tolerates deferred optional collections

Given a phase-1 governed data source that contains the required about group, header group, and ordered resume sections but no separate skills taxonomy, relationships collection, or rich metadata beyond simple provenance

When the app loads and transforms that source into the resume data contract

Then the load succeeds, profile, sections, and items are populated from governed data, the deferred skills and relationships collections and any rich metadata beyond simple provenance behave as empty, and phase-1 completion does not require inventing taxonomy or relationship data with no governed source

## Scenario: Search projection is generated without a browser DOM

Given a governed resume item description contains markdown or HTML syntax and plain-text prose, and the search projection is generated during server-side or build-time rendering where `document` and `window` are unavailable

When the app produces the plain-text projection used for interactive search

Then the projection strips markdown and HTML syntax from searchable content, preserves the plain-text prose that visitor search matches, and is produced without requiring a browser DOM dependency
...
Then on the narrow viewport the navigation controls collapse behind a toggle without horizontal scroll, and on the wide viewport search, Contents, Skill Levels, and Reset appear inline with Instructions, About, and the Static link (to `/static`) right-aligned
...
When the visitor searches for `validated` and separately searches for `theleanstartup` in interactive mode

Then `validated` matches exactly the `Growth / Lean` item in the `Skills/Tools - Methodologies/Processes` section and no other item, while `theleanstartup` matches no items because it appears only inside markdown URL syntax
...
## Scenario: Invalid skill level is rejected at load

Given a governed data source with a section item whose `level` is present but is not one of the five defined skill-level keys

When the app loads that governed data

Then the app rejects the source as malformed rather than rendering the item — failing the build/prerender under the phase-1 build-time load so the invalid data never ships, or rendering the visitor-safe error state under a runtime load path
...
Then the two-position date column renders exactly as the predecessor: a present start renders as `M.YYYY` followed immediately by the predecessor's non-breaking-space-and-hyphen separator (for example `7.2001&nbsp;-`) in the start position, a present end renders as its bare `M.YYYY` in the end position (for example the `Senior Software Engineer, Pivotal` item with start `2006-04-15` and end `2019-10-29` renders `4.2006&nbsp;-` in the start position and `10.2019` in the end position), a missing start with a present end renders `until` in the start position, a present start with a missing end renders `current` in the end position, and a missing start and end renders the item as `current` in the end position with nothing in the start position

## Scenario: Markdown renders consistently across modes

Given governed `about.content` and item descriptions contain markdown using headings, unordered lists, paragraphs, emphasis, strong, inline links, inline code spans such as `` `process_helper` ``, bare URLs such as `https://github.com/thewoolleyman/process_helper`, and owner-authored raw HTML

When the resume renders in interactive mode and in static mode

Then both modes use the same markdown renderer and configuration, the rendered output is byte-identical across modes, all of those markdown features render, the inline code span renders as a code element, bare URLs render as links with trailing sentence punctuation outside the link target, and the owner-authored raw HTML is preserved rather than stripped under the phase-1 trusted-source posture
