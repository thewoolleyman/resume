...
**Committed production snapshot.** The predecessor's production resume content MUST be transcribed into `data/resume.yml` so predecessor parity is preserved rather than lost. The predecessor served its production content from an external YAML data source at `https://interactive-resume-data-chad-woolley.gitlab.io/interactive-resume-data-chad-woolley.yml`; that production content is the material to import, not the predecessor repository's development or test fixtures. A snapshot of that production content MUST be committed into `data/resume.yml` as the authoritative import material so phase 1 does not depend on a live external fetch. The committed snapshot MAY diverge from the retrieved source through recorded owner-directed edits and therefore need NOT be byte-identical to the retrieved source. Exactly two edit categories are sanctioned: (1) **PII redactions** applied for privacy (for example, removing the owner's postal address from `header.contact`), which remove characters within a preserved field's value; and (2) **owner-authored content edits** to the app-meta fields `about.title` and the markdown `about.content` (and, where the owner explicitly directs it, an individual resume item's factual fields — its display `name`, its `start` and `end` dates, and its markdown description — to record a real-world career change such as a role retitle or a departure end date), which — unlike a redaction — MAY add, rewrite, or delete field VALUES so the content intentionally departs from the predecessor import (see the About app-meta content provision below). An owner-authored edit to an item's display `name` re-derives that item's stable identifier and public anchor per §"Stable item identifiers", which is the intended behavior when the display name legitimately changes. Neither category may drop or add any top-level key, section, or item: the pinned production scope below is UNCHANGED by either edit, the YAML data MUST still expose exactly the production top-level keys named in the pinned production-scope paragraph below, and provenance comments MUST NOT add a parsed top-level data group. Whether or not any edit is applied, the repository MUST record the provenance in leading YAML comments in that same file: (a) the retrieved-source provenance — the source URL, the retrieval date, the upstream `Last-Modified` date, and the SHA-256 of the retrieved bytes (the immutable pre-edit predecessor hash); (b) every owner-directed edit — each redaction and each owner-authored content edit — with its date and a one-line description of what changed; and (c) the committed snapshot's OWN SHA-256, computed over the current on-disk content. That committed-snapshot SHA-256 MUST be re-pinned identically after ANY recorded owner-directed edit — not only a PII redaction — in those provenance comments, in `src/lib/data/import.test.ts`, and in this spec; the retrieved-source SHA-256 is immutable predecessor provenance and never changes. The pinned provenance of the current snapshot is: retrieved-source SHA-256 `792097b01aef31fdc7cbf2c2463492e87c5ca89bc8d864ef3ebacfc7f7a4d158` (source `https://interactive-resume-data-chad-woolley.gitlab.io/interactive-resume-data-chad-woolley.yml`, retrieved 2026-07-06, upstream `Last-Modified` 2022-06-27); and committed-snapshot SHA-256 `61e2b2ca076b56686a871854eff3b31f37b9c484ef7727c44f652e2675d4a78a`, reflecting four owner-directed edits — three on 2026-07-10 (the redaction of the postal address from `header.contact` (leaving phone and email); the owner-authored rewrite of `about.content` to describe the current implementation and scrub the retired GitLab-era self-description; and the owner-authored update of the current GitLab role (retitled Senior→Staff Fullstack Engineer, added the `2026-06-15` end date, and rewrote its description) to record the owner's departure) and one on 2026-07-11 (neutralizing the `about.content` opening line from "interactive resume" to "resume" so the shared About reads correctly on the static view as well as the interactive view).

**About app-meta content (owner-authored).** The `about` group is a pinned top-level key that counts toward the 18-key / 74-item production scope, but its BODY — `about.title` and the markdown `about.content` — is owner-authored self-description of THIS app, not a predecessor career fact. Its parity obligation is therefore STRUCTURAL (the `about` key exists with a title and markdown content) rather than TEXTUAL: `about.title` and `about.content` are carved out of the strict predecessor-text freeze that governs the career sections, and the owner MAY author them via an owner-authored content edit (above) to track the current implementation. Accordingly, `about.content` MUST describe the CURRENT implementation — the SvelteKit-on-Vercel app, GitHub-hosted source and CI, the Bun/Vitest/Playwright toolchain, and its AI/livespec-driven authoring and Claude-driven design — rather than reproducing the predecessor's retired Vue/Nuxt/GitLab-infrastructure and Bootstrap self-description, while retaining a brief historical note that the resume was hand-coded in Vue in 2019 to land the GitLab role and converted to an AI-driven project in 2026 via the owner's own livespec project and factory tooling. `about.content` MUST NOT reference the retired cover-letter host `cover-letter.thewoolleyweb.com`, MUST NOT frame the resume as an application to a specific employer, and — consistent with §"Resume data" — MUST NOT link to retired or ungoverned off-site surfaces.
...
- The `about.title` and markdown `about.content` fields (their presence is preserved; their text is owner-authored app-meta per the About app-meta content provision above, not frozen predecessor text).
...
- Within each section, every item, its order, its display name, its optional skill `level`, its optional `start` value, its optional `end` value, and its markdown description. (Per the sanctioned owner-authored content-edit category above, the owner MAY revise an individual item's display name, `start`/`end` dates, or markdown description; such an edit changes field VALUES only and still preserves the item, its order, and its `level` presence.)
...
- An `about` group with a title and markdown content (`about.title`, markdown `about.content`). The `about` group is a pinned structural key, but its title and markdown body are owner-authored app-meta whose parity is structural, not textual (see the About app-meta content provision in §"Governed data source and predecessor import (phase 1)"); the owner MAY author them to describe the current app.
