...
The static resume (`/static`) MUST include a link back to the interactive resume (`/`), rendered as the interactive resume's full canonical URL so a printed or PDF copy of the static resume remains actionable. The interactive resume already links forward to `/static` via its Static control (§"Layout and controls"). Because the shared `about.content` is owner-authored app-meta rendered byte-identically in both modes (§"Item rendering"), it MUST NOT frame itself as belonging to only one mode (for example, it MUST NOT call the resume "my interactive resume"); the mode-specific framing is carried by this cross-link chrome, not by the shared About body.
...
- **Sticky, responsive navigation bar.** The navigation bar MUST be sticky and dark-themed. On narrow viewports it MUST collapse its controls behind a toggle rather than overflowing or forcing horizontal scroll; on wide viewports the controls MUST be laid out inline. Any dropdown menu the bar exposes (Contents, Skill Levels) MUST dismiss when the visitor interacts outside it — an outside pointer press, the Escape key, or moving keyboard focus away from the menu — rather than remaining open until it is re-toggled.
- **Control order.** The navigation bar MUST include, in this order, live search, Contents, Skill Levels, and Reset, with Instructions, About, and a Static link right-aligned at the trailing edge of the bar, in that order.
- **Static link.** The trailing edge MUST include a Static control that navigates to the `/static` route (the traditional static resume rendering per §"Web routes"). Because it is a route link, it MUST be a real anchor carrying an `href` to `/static` — so it works without JavaScript and is crawlable — rather than a scripted-only control, and it is styled as a prominent nav affordance distinct from the toggle controls.
...
A present item `level` MUST be one of the five defined keys, or the governed source is rejected at load as malformed (per §"Governed data source contract"); there is no runtime tolerance for an invalid level. A no-level item behaves as `unspecified` and renders no badge (as above). The Skill Levels control still defaults to all levels selected and toggles each level independently.
...
**Markdown rendering and sanitization.** Markdown in `about.content` and item `desc` MUST render consistently across interactive and static modes: both modes MUST use the same markdown renderer and configuration so the rendered output is byte-identical between modes. The renderer MUST support the markdown features the pinned production source uses: headings, unordered lists, paragraphs, emphasis and strong, inline links, inline code spans, and bare URL autolinks. Inline code spans MUST render as code elements, matching the predecessor renderer; for example, the production span `` `process_helper` `` renders as `<code>process_helper</code>`. Bare `http://` and `https://` URLs MUST autolink with predecessor-compatible behavior; trailing sentence punctuation that is not part of the URL, such as a final period or comma, MUST remain outside the link target. Because every phase-1 governed source is owner-authored and therefore trusted, raw HTML embedded in governed markdown MUST be preserved and rendered (matching predecessor output) rather than stripped; the requirement to sanitize "before any untrusted content is admitted" is a forward guard for a future phase that admits non-owner content and is NOT triggered in phase 1. Should a later phase admit untrusted content, a documented sanitization allowlist MUST be applied before that content is rendered.
