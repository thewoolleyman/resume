...
So this parity does not depend on a live external fetch, a snapshot of the predecessor's production content MUST be committed into `data/resume.yml` as the authoritative import material; the committed `data/resume.yml` snapshot, not the live URL, is what the build reads. Per `spec.md` §"Governed data source and predecessor import (phase 1)", that committed snapshot MUST be transcribed from the predecessor's production content but need NOT be byte-identical to the retrieved source: it MAY diverge through owner-directed PII redactions and owner-authored content edits recorded in that file's leading provenance comments. Its provenance MUST follow the two-hash model defined there — the immutable retrieved-source SHA-256 (alongside the source URL, retrieval date, and upstream `Last-Modified`) plus the committed-snapshot SHA-256, together with the per-edit ledger — rather than a single content hash. This boundary cross-references that spec.md provision rather than restating it and MUST NOT assert a stricter byte-verbatim or single-hash rule.
